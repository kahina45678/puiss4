    ...
    on_start_click = async () => {
        ...
        try {
            let response = await fetch(`/game/new_game`, {
                ...
                body: JSON.stringify({
                    mode: this.state.select === '1v1' ? 'multi' : 
                          this.state.select === 'AI' ? 'single' : 'ai_vs_ai'
                })
        ...
        return (
            <div id="form">
                <div id="mode">
                    ...
                    <GameModeButton on_select={this.on_select} select={this.state.select}>ai vs ai</GameModeButton>
                    {/* <GameModeButton on_select={this.on_select} select={this.state.select}>AI vs AI</GameModeButton> */}
